<script>
	import { loaded, findMovies, fetchFindMovies } from '../foundstore';
    ...
    async function handleSearch() {
        if (!searchTerm) {
            const res = await await fetch(`https://api.themoviedb.org/3/search/multi?api_key=${api_key}&language=pl&query="Spiderman"`);
            const data = await res.json()
            foundMovies = data.results
            return data.results;
        }

        const res = await fetch(`https://api.themoviedb.org/3/search/multi?api_key=${api_key}&language=pl&query=${searchTerm}`)
        const data = await res.json()
        console.log(data)
        foundMovies = data.results
        return foundMovies
